...
func (session *PullSession) connect(rawUrl string) (err error) {
	...
	session.urlCtx, err = base.ParseHttpflvUrl(rawUrl, isHttps)
	if err != nil {
		return
	}
	nazalog.Debugf("[%s] > tcp connect.", session.uniqueKey)

	nazalog.Debugf("[%s] > urlCtx info.", session.urlCtx)
	// # 建立连接
	var conn net.Conn
	if isHttps{
		conn, err = tls.Dial("tcp", session.urlCtx.HostWithPort, &tls.Config{InsecureSkipVerify: true})
	}else {
...
func (session *PullSession) writeHttpRequest() error {
	...
	nazalog.Debugf("req:%s", req)
	nazalog.Debugf("scheme:%s", session.urlCtx.Scheme)
...
func (session *PullSession) readHttpRespHeader() (statusLine string, headers map[string]string, err error) {
	...
	nazalog.Debugf("resp:%s", statusLine)
	nazalog.Debugf("headers:%s", headers)
